# 🎴 FlashcardDAS – Flashcard Quiz Program

## Description
**FlashcardDAS** is a Python-based console quiz application that presents users with a **random set of seven flashcard questions** from an Excel file.

The user is first asked whether they want to play the flashcard quiz. If they choose to continue, they can select one of three categories:
- Biology
- Geography
- History
...
## Program Flow
A visual overview of the program logic is provided in the flowchart:
`images/Flashcards_Flowchart.png`
...
## Excel File Format
The Excel file `FlashcardsDAS.xlsx` contains one sheet per category:
- Biology
- Geography
- History1 (named this way because "History" is reserved as a sheet name in Excel)
...
```bash
python FlashcardDAS.py
```

or, if required:

```bash
python3 FlashcardDAS.py
```
...
- Invalid category numbers trigger a custom `InvalidCategoryError`
- Non-numeric input is handled using `try/except` (`ValueError`)
- A missing Excel file raises a clear `FileNotFoundError`

## Random Question Module
The module `Random_question.py`:
- Reads quiz data from `FlashcardsDAS.xlsx`
- Selects seven random questions using `DataFrame.sample()`
